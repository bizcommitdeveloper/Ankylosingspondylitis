...
export default function LoginScreen() {
  ...
  const { user, loading, configured, googleConfigured, signInWithEmail, registerWithEmail } = useAuth();
    ...
    <ScrollView style={{ flex: 1, backgroundColor: t.bg }} contentContainerStyle={{ padding: 20, gap: 12 }} keyboardShouldPersistTaps="handled">
      ...
      {!configured ? (
        ...
        <>
          ...
          <TextInput value={email} onChangeText={setEmail} placeholder="Email" placeholderTextColor={t.muted} autoCapitalize="none" keyboardType="email-address" autoComplete="email" style={inputStyle} />
          <TextInput value={password} onChangeText={setPassword} placeholder="Password" placeholderTextColor={t.muted} secureTextEntry autoComplete="password" style={inputStyle} />

          <Button title={busy ? "Please wait…" : mode === "register" ? "Create account" : "Sign in"} onPress={handleEmail} loading={busy} />

          <Text
            onPress={() => { setMode(mode === "register" ? "signin" : "register"); setError(null); }}
